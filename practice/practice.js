...
console.log('');
    ...
    let phi = table => {
        return ((table[2] * table[0]) - (table[3] * table[1])) / 
        Math.sqrt(
...
const SCRIPTS = [
  ...
  console.log(filter(SCRIPTS, testElement));

  // using built-in array filter() method
  console.log('// using built-in array filter() method')
  console.log(SCRIPTS.filter(e => e.living == true));

  // transforming with map
  console.log('// transforming arrays with map')
  ...
  let names = SCRIPTS.filter(el => el.direction === 'rtl');
  console.log(map(names, (el) => el.name));

  // Summarizing with reduce 
  console.log('// summarizing arrays with reduce method');
// NB: reduce can also be termed as fold.
...
console.log('// using the es6 reduce() method');
console.log([2,4,6,8,10].reduce((start, combine) => start + combine));

// using reduce twice (flatening array of arrays)
...
console.log('// Finding the average year of origin for living and dead scripts');
function averageYear(array){
  return array.reduce((a, b) => a + b) / array.length;
}
console.log('Average year of origin for living scripts (filter(), map() and reduce())')
console.log(Math.round(averageYear(SCRIPTS.filter(s => s.living).map(s => s.year))));

console.log('Implementing with one big loop');
function findLivingAverage(){
  let total = 0, count = 0;
  for(let script of SCRIPTS){
    if(script.living){
      total += script.year;
...
console.log(Math.round(findLivingAverage()));

console.log('Average year of origin for dead scripts (filter(), map() and reduce())');
console.log(Math.round(averageYear(SCRIPTS.filter(s => !s.living).map(s => s.year))));

console.log('Implementing with one big loop');
function findDeadAverage(array){
  let total = 0, count = 0;
